...
$(function () 
{  
     show_products_data(type);
});

function show_products_data(type)
{
  $.ajax({
     url: base_url + 'show_products_data',
        type: "POST",
         data: {type : type},
        dataType: "json",
        ...
        success: function (success) 
        {
            ...
            if (success.status === 'success') 
            {
                ...
                for(i=0;i<main_data.length;i++)
                {
                    if(main_data[i]['gst_type'] == '0'){
                     ...
                     if(main_data[i]['statusP'] == '0'){
                        status = `<a href="`+base_url + 'inactiveproducts'+`/`+main_data[i]['Product_id']+`" class="btn btn-sm btn-success" onclick="return confirm ('Are You Sure? Inactive this Product')">Active</a>`;
                    }
                    else{
                ...
                table+=`
                     <tr>
                       <td><input type="checkbox" name="inactivebulk[]" id="inactivebult" value="`+main_data[i]['Product_id']+`" ></td>
                       <td>`+k+++`</td>
                        <td><a href="`+success.img[i]+`" target="_blank"><img src="`+success.img[i]+`" height="50" width="50"></a></td>
                       <td>`+main_data[i]['ProductName']+`<br>( SKU : `+main_data[i]['sku_code']+` )</td>
                       ...
                       <td>
                           <a href="#" class="call h5 mb-0" id="editproducts" data-id="`+main_data[i]['Product_id']+`" ><i class="icon-pencil"></i></a> 
                             <a href="`+base_url + 'deleteproducts'+`/`+main_data[i]['Product_id']+`" class="call h5 mb-0"><i class="icon-trash" onclick="return confirm ('Are You Sure? Delete this Product')"></i></a> 
                           </td>
                        <td>`+status+`</td>
                      </tr>`;    
                }
...
$(document).on('click', '#editproducts', function(){
    ...
    // 			console.log(response);
			    ...
			    $('#hns_code1').val(response.product.hns_code);
			    $('#minqty').val(response.product.MinimumQty);
...
$("#piece_price").change(function(){
    	var selling_price =  $('#piece_price').val();
    	var mrp =  $('#mrp').val();
...
// $('#mini_quantitty_bind').keyup(function(){
//     var mini_quantity_bind = $('#mini_quantitty_bind').val();
...
// });
  ...
  $('#set_unit,#unit_set,#piece_price,#unit_select').on('change', function() {
    ...
    var selling_price_set_1 = Number(piece_price) - Number(discount_mrp_1);
    var selling_price_set_2 = Number(piece_price) - Number(discount_mrp_2);
    var selling_price_set_3 = Number(piece_price) - Number(discount_mrp_3);

    $( "#set_price" ).val(piece_price*unit_set);
    $( "#row1col1" ).text(setdata+' ' +value1);
    ...
    $( "#row1col4" ).text('1 '+unit_select+' = ₹' +(piece_price-discount_mrp_1)*unit_set);
    $( "#row2col4" ).text('1 '+unit_select+' = ₹' +(piece_price-discount_mrp_2)*unit_set);
    $( "#row3col4" ).text('1 '+unit_select+' = ₹' +(piece_price-discount_mrp_3)*unit_set);
...
//     $( "#selling_price" ).val(piece_price*unit_set);
//     $( "#row1col1" ).text(setdata+' ' +value1);

// });


$('#piece_price').on('change', function() {
    var value = $( this ).val();
...
$(document).on('change', '#mini_quantitty_bind',  function(){
    i=0;
    i++;
    var value1 = $('#set_unit option:selected').text();
...
$(document).on('change', '#maximum_quantity',  function(){
    ...
    var mini1 = $("#mini_quantitty_bind").val();
    if(mini1 == 0 || mini1 == ''){
        alert('Please Check Maximum Quantity');
...
$(document).on('keyup', '#discount_mrp_3', function(){  
  ...
  $('#set_unit1,#unit_set1,#selling_price1,#unit_select1').on('change', function() {
    ...
    var unit_set = $('#unit_set1').val();
    var piece_price =  $('#selling_price1').val();
    var value1 = $('#set_unit1 option:selected').text();
    ...
    $( "#set_price1" ).val(piece_price*unit_set);
    ...
    $( "#row11col4" ).text('1 '+unit_select+' = ₹' +(piece_price-discount_mrp_1)*unit_set);
    $( "#row22col4" ).text('1 '+unit_select+' = ₹' +(piece_price-discount_mrp_2)*unit_set);
    $( "#row33col4" ).text('1 '+unit_select+' = ₹' +(piece_price-discount_mrp_3)*unit_set);
...
if(confirm("Are you sure want to delete these Products?")) {
    var id =  $('#inactivebult').val();
    var myCheckboxes = new Array();
    $("#inactivebult:checked").each(function() {
    data['inactivebult[]'].push($(this).val());
});
